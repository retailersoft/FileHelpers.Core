...
namespace FileHelpers
{
        ...
        : IRecordInfo
    {
        ...
        {
            TypedRecordAttribute recordAttribute = Attributes.GetFirstInherited<TypedRecordAttribute>(RecordType) ?? throw new BadUsageException($"The record class {RecordType.Name} must be marked with the [DelimitedRecord] or [FixedLengthRecord] Attribute");
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            List<FieldInfo> fields = new(ReflectionHelper.RecursiveGetFields(RecordType));
        ...
        private static void CheckForOptionalAndArrayProblems(List<FieldBase> resFields)
        {
            for (int i = 0; i < resFields.Count; i++) {
                FieldBase currentField = resFields[i];
        ...
        private static void CheckForOrderProblems(FieldBase currentField, List<FieldBase> resFields)
        {
            if (currentField.FieldOrder.HasValue) {
                ...
                FieldBase fieldWithoutOrder = resFields.Find(x => x.FieldOrder.HasValue == false);
                ...
                FieldBase fieldWithOrder = resFields.Find(x => x.FieldOrder.HasValue);
                if (fieldWithOrder != null)
                {
                    string autoPropertyName = FieldBase.AutoPropertyName(currentField.FieldInfo);
