...
namespace FileHelpers.Core
{
    ...
    internal sealed class RecordOperations
    {
        ...
        {
            StringBuilder sb = new(mRecordInfo.SizeHint);

            object[] values = ObjectToValuesHandler(record);
        ...
        public DataTable CreateEmptyDataTable()
        {
            ...
            foreach (FieldBase f in mRecordInfo.Fields) {
                DataColumn column1;
                if (f.IsNullableType) {
        ...
        {
            FieldInfo[] res = new FieldInfo[mRecordInfo.Fields.Length];
