...
namespace FileHelpers.Core.DataLink
{
    ...
    public abstract class DatabaseStorage : DataStorage
    {
        ...
        private IDbTransaction InitTransaction(IDbConnection conn)
        {
            ...
            return mTransactionMode switch
            {
                TransactionMode.UseDefault => conn.BeginTransaction(),
                TransactionMode.UseChaosLevel => conn.BeginTransaction(IsolationLevel.Chaos),
                TransactionMode.UseReadCommitted => conn.BeginTransaction(IsolationLevel.ReadCommitted),
                TransactionMode.UseReadUnCommitted => conn.BeginTransaction(IsolationLevel.ReadUncommitted),
                TransactionMode.UseRepeatableRead => conn.BeginTransaction(IsolationLevel.RepeatableRead),
                TransactionMode.UseSerializable => conn.BeginTransaction(IsolationLevel.Serializable),
                _ => null,
            };
        }
