...
namespace FileHelpers
{
        ...
        where T : class, IComparable<T>
    {
        ...
        public void Sort(string sourceFile, string destinationFile)
        {
            ...
            List<string> parts = [];
            FileHelperAsyncEngine<T> engine = SplitAndSortParts(sourceFile, parts);
            SortQueue<T>[] queues = CreateQueues(parts);
        ...
        {
            SortQueue<T>[] res = new SortQueue<T>[parts.Count];
        ...
        private FileHelperAsyncEngine<T> SplitAndSortParts(string file, List<string> res)
        {
            ...
            try {
                ...
                FileHelperAsyncEngine<T> readEngine = new(Encoding);
        ...
        {
            string splitName = GetSplitName(file, partNumber);
            res.Add(splitName);
            ...
            FileHelperEngine<T> writeEngine = new(Encoding) {
                Options = {
                    IgnoreFirstLines = 0,
                    IgnoreLastLines = 0
                }
        ...
        internal void MergeTheChunks(SortQueue<T>[] queues, string destinationFile, string headerText, string footerText)
        {
            try {
                // Merge!
                using FileHelperAsyncEngine<T> sw = new(Encoding);
