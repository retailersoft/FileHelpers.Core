...
namespace FileHelpers.Core
{
    ...
    public static class CommonEngine
    {
        ...
        public static int TransformFileFast<TSource, TDest>(string sourceFile, string destFile)
            ...
            where TDest : class
        {
            FileTransformEngine<TSource, TDest> engine = new();
        ...
        {
            FileHelperEngine engine = new(recordClass);
            FieldInfo fi = engine.RecordInfo.GetFieldInfo(fieldName) ?? throw new BadUsageException("The record class does not contain the field " + fieldName);
            object[] res = engine.ReadFile(sourceFile);
        ...
        public static void SortRecordsByField(object[] records, string fieldName, bool ascending)
        {
            if (records.Length > 0 &&
                records[0] != null) {
                FileHelperEngine engine = new(records[0].GetType());
                FieldInfo fi = engine.RecordInfo.GetFieldInfo(fieldName) ?? throw new BadUsageException("The record class does not contain the field " + fieldName);
        ...
        {
            using FileHelperAsyncEngine engineRead = new(recordType);
            using FileHelperAsyncEngine engineWrite = new(recordType);
        ...
        public static IEnumerable<RecordIndexer> ReadCsv(string filename,
            ...
            Encoding encoding)
        {
            FileHelperAsyncEngine<RecordIndexer> engine = new(encoding);
        ...
        public static void SortBigFile<T>(Encoding encoding, string source, string destination)
            where T : class, IComparable<T>
        {
            BigFileSorter<T> sorter = new(encoding);
            sorter.Sort(source, destination);
        }
    }
}
