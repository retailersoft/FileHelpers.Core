...
namespace FileHelpers.Core
{
        ...
        where T : class
    {
        ...
        {
            using InternalStreamReader fs = new(fileName, mEncoding, true, DefaultReadBufferSize);
            T[] tempRes;
        ...
        {
            using InternalStreamReader fs = new(fileName, mEncoding, true, DefaultReadBufferSize);
            List<T> res = ReadStreamAsList(fs, maxRecords);
            fs.Close();
            return res;
        }
        ...
        {
            IList result = ReadStreamAsList(reader, maxRecords, null);
        ...
        private IList ReadStreamAsList(TextReader reader, int maxRecords, DataTable dt)
        {
            ...
            StreamInfoProvider streamInfo = new(reader);
            using (ForwardReader freader = new(recordReader, RecordInfo.IgnoreLast))
            {
        ...
        public T[] ReadString(string source, int maxRecords)
        {
            ...
            using InternalStringReader reader = new(source);
        ...
        public List<T> ReadStringAsList(string source, int maxRecords)
        {
            ...
            using InternalStringReader reader = new(source);
            List<T> res = ReadStreamAsList(reader, maxRecords);
            reader.Close();
            return res;
        }
