...
namespace FileHelpers
{
    ...
    public abstract class FieldBase
    {
        ...
        public static FieldBase CreateField(FieldInfo fi, TypedRecordAttribute recordAttribute)
        {
            FieldBase res = null;
            MemberInfo mi = fi;
            string memberName = "The field: '" + fi.Name;
            Type fieldType = fi.FieldType;
            string fieldFriendlyName = AutoPropertyName(fi);
            if (string.IsNullOrEmpty(fieldFriendlyName) == false)
            {
                PropertyInfo prop = fi.DeclaringType.GetProperty(fieldFriendlyName);
                if (prop != null)
                {
...
#pragma warning restore 612,618
            ...
            FieldAttribute[] attributes = (FieldAttribute[])mi.GetCustomAttributes(typeof(FieldAttribute), true);
            ...
            if (attributes.Length > 0)
            {
                ...
                if (fieldAttb is FieldFixedLengthAttribute attribute)
                {
                    // Fixed Field
                    if (recordAttribute is DelimitedRecordAttribute)
                    {
                    ...
                    FieldFixedLengthAttribute attbFixedLength = attribute;
                    FieldAlignAttribute attbAlign = Attributes.GetFirst<FieldAlignAttribute>(mi);
                ...
                else if (fieldAttb is FieldDelimiterAttribute attribute1)
                {
                    // Delimited Field
                    if (recordAttribute is FixedLengthRecordAttribute)
                    {
            ...
            {
                if (recordAttribute is DelimitedRecordAttribute delimitedRecordAttribute)
                {
                    res = new DelimitedField(fi,
        ...
        internal static string AutoPropertyName(FieldInfo fi)
        {
            if (fi.IsDefined(typeof(CompilerGeneratedAttribute), false))
            {
                if (fi.Name.EndsWith("__BackingField") &&
                    ...
                    return fi.Name[1..fi.Name.IndexOf('>')];
        ...
        internal object ExtractFieldValue(LineInfo line)
        {
            ...
            else
            {
                ...
                ArrayList res = new(Math.Max(ArrayMinLength, 10));
                       ...
                       i < ArrayMaxLength)
                {
                    ...
                    try
                    {
                        AssignResult value = AssignFromString(info, line);
        ...
        {
            string extractedString = fieldString.ExtractedString();
            ...
            try
            {
                ...
                {
                    string trimmedString = extractedString.Trim();
        ...
        private string TrimString(string extractedString)
        {
            ...
            {
                TrimMode.None => extractedString,
                TrimMode.Both => extractedString.Trim(),
                TrimMode.Left => extractedString.TrimStart(),
                TrimMode.Right => extractedString.TrimEnd(),
                _ => throw new Exception("Trim mode invalid in FieldBase.TrimString -> " + TrimMode.ToString()),
            };
        }
